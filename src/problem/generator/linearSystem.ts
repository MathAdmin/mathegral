...
const level1 = (): Problem => {
  ...
  const problem = b1 * c2 - b2 * c1 === 0 ? INFINITY : undefined;

  return {
    terms: [
      { a: 0, b: b1, c: c1, d: 0, e: e1 },
...
const level2 = (): Problem => {
  ...
  //Parameter
  const [b1, b2, b3, c1, c2, c3, d1, d2, d3] = randomInts(
    9,
    -15,
    16,
    exclude(0)
  ...
  const problem = determinant === 0 ? INFINITY : undefined;

  return {
    terms: [
      { a: 0, b: b1, c: c1, d: d1, e: e1 },
...
export const formatSolution = (
  ...
  const sols = problem.problem
    ? `\\text{${translate(
        `generator.linear-system.problem.${problem.problem}`
      )}}`
    : `\\mathbb{L}=\\{(` +
      [
        ...Array.from(problem.solutions.map((sol) => fracTex(sol.a, sol.b))),
      ].join(";") +
      `)\\}`;
